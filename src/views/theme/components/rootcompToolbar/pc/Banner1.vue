...
<template>
  <div class="editorBannerBox">
      <h2>插图</h2>
      <ul class="editorList">
        <draggable v-model="activeComponent.componentContent.bannerList2">
        <li
        class="item"
        v-for="(item, index) in activeComponent.componentContent.bannerList2"
        :key="index"
        >
        ...
        <div
        class="addImgBtn"
        @click="addImgText(activeComponent.componentContent.bannerList2.length)">
...
export default {
  ...
  methods: {
    ...
    addImgText (index) {
      console.log(index)
      this.activeComponent.componentContent.bannerList2.push({
        title: `轮播${index + 1}`,
        des: '',
        pic: '',
        url: '',
    ...
    deleteItem (item, index) {
        this.$confirm('确定删除此项？')
        .then((_) => {
            this.activeComponent.componentContent.bannerList2.splice(index, 1)
        })
        .catch((_) => {})
    },
    deleteIt (index) {

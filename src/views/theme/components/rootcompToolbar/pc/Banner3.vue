...
<template>
  <div class="editorBannerBox">
    <h2>插图</h2>
    <ul class="editorList">
      <draggable v-model="activeComponent.componentContent.bannerList4">
        <li
          class="item"
          v-for="(item, index) in activeComponent.componentContent.bannerList4"
          :key="index"
        >
          <DraggableSlot :obj="item" :index="index" @deleteIt="deleteIt">
            <div class="addContent">
              ...
              <el-input placeholder="描述" resize="none" v-model="item.des" />
...
export default {
  ...
  methods: {
    ...
    addImgText(index) {
      console.log(index);
      this.activeComponent.componentContent.bannerList4.push({
        title: `轮播${index + 1}`,
        des: "",
        pic: "",
        url: "",
    ...
    deleteItem(item, index) {
      this.$confirm("确定删除此项？")
        .then((_) => {
          this.activeComponent.componentContent.bannerList4.splice(index, 1);
        })
        .catch((_) => {});
    },
    deleteIt(index) {
